...
import { useRaids, useRaidsBySubcategory } from '../lib/hooks';
import { raidsSubcategories } from '../lib/api';
...
const ErrorState = () => (
  <Card className="p-8 text-center">
    ...
    <h3 className="text-xl font-bold text-slate-300 mb-2">Error loading raids</h3>
...
const SubcategoryView = ({ subcategory }: { subcategory: string }) => {
  const { data: raids, loading, error } = useRaidsBySubcategory(subcategory);
  ...
  return (
    <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
                ...
                <div className="p-5 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-slate-500 text-xs">Min Level: {raid.min_level}</p>
                    </div>
                  ...
                  <div className="flex-grow">
                    {raid.stats && !raid.subraids?.length && (
                        ...
                        {raid.difficulty === 'Normal' ? (
                          <div className="grid grid-cols-2 gap-3 p-3 bg-slate-800/50 rounded-lg border border-slate-700/50">
                            ...
                            <div>
                              ...
                              <p className="text-amber-400 font-medium text-xs italic">{raid.stats.importantDrop || 'none'}</p>
                            </div>
                          </div>
                        ) : (
                          <div className="space-y-3">
                            ...
                            {raid.stats.importantDrop && raid.stats.importantDrop !== 'none' && (
                              <div className="p-3 bg-amber-500/10 rounded-lg border border-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.05)]">
                                ...
                                <div className="space-y-1.5">
                                  {raid.stats.importantDrops && raid.stats.importantDrops.length > 0 ? (
                                    // Use DropItem component when we have importantDrops with images
                                    raid.stats.importantDrops.map((drop, index) => (
                                      <DropItem key={index} drop={drop} />
                                    ))
                                  ) : (
                                    // Fallback to plain text when no importantDrops array
                                    raid.stats.importantDrop.split(/,|\s-\s/).map((dropName, index) => (
                  ...
                  {raid.subraids && raid.subraids.length > 0 && (
        ...
        {allRaids.length === 0 && subcategory !== 'mythic' && (
          <div className="text-center py-16">
            <div className="text-5xl mb-4">🏰</div>
            <h3 className="text-xl font-bold text-slate-300 mb-2">No raids found</h3>
            <p className="text-slate-500">Raids for this category are coming soon!</p>
          </div>
        )}
      </div>
    </div>
...
// Main raids view component
const MainRaidsView = () => {
  const { loading, error } = useRaids();
