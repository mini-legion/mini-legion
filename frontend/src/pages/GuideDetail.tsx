...
export const GuideDetail = () => {
  ...
  const renderSection = (section: GuideSection, index: number) => {
    switch (section.type) {
      ...
      case "video": {
        const embedUrl = getYouTubeEmbedUrl(section.videoUrl || section.content);
        if (!embedUrl) return null;

        return (
          <div key={index} className="guide-section">
            {section.title && <h2 className="guide-heading">{section.title}</h2>}
            <div className="rounded-2xl border border-red-500/20 bg-gradient-to-br from-slate-900 to-slate-950 p-4 shadow-xl shadow-red-500/10">
              ...
              {(section.videoTitle || section.videoCreator || section.content) && (
                <div className="mt-4">
                  {section.videoTitle && <h3 className="text-lg font-black text-slate-100">{section.videoTitle}</h3>}
                  {section.videoCreator && (
                    <p className="mt-1 text-sm text-slate-400">
                      Video by{' '}
                      {section.videoCreatorUrl ? (
                        <a href={section.videoCreatorUrl} target="_blank" rel="noopener noreferrer" className="font-bold text-amber-400 hover:text-amber-300">
                          {section.videoCreator}
                        </a>
                      ) : (
                        <span className="font-bold text-amber-400">{section.videoCreator}</span>
                      )}
                    </p>
                  )}
                </div>
