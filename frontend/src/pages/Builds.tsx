...
export const Builds = () => {
  ...
  if (subcategory) {
    const currentSubcategory = buildsSubcategories.find(
      (s) => s.id === subcategory
    );
    ...
    return (
      <div>
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden">
          <div
            className={`absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-br ${colors.gradient} opacity-5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3`}
          />
          <div
            className={`absolute bottom-0 left-0 w-[400px] h-[400px] bg-gradient-to-br ${colors.gradient} opacity-3 rounded-full blur-3xl translate-y-1/2 -translate-x-1/3`}
          />
        </div>
          ...
          {builds.length > 0 ? (
                      ...
                      <div className="p-6">
                        <div className="flex items-start gap-4 mb-4">
                          <div className="w-24 h-24 flex items-center justify-center text-4xl group-hover:scale-110 transition-transform duration-300 shrink-0">
                            {colors.icon.length > 4 ||
                            colors.icon.startsWith("/") ? (
                              <img
                                src={colors.icon}
                                alt={build.spec}
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-2xl font-bold text-slate-300 mb-3">
                {currentSubcategory?.name} Builds Coming Soon
              </h3>
              <p className="text-slate-500 max-w-md mx-auto">
                We're working on adding the best builds for this class. Check
                back soon!
              </p>
              <Link
                to="/builds"
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {buildDetails.slice(0, 6).map((build) => {
              const colors =
                classColors[build.heroClass.toLowerCase()] ||
                classColors.hunter;
              return (
                <Link key={build.id} to={`/builds/detail/${build.id}`}>
                  <Card className="overflow-hidden group h-full" glow="amber">
                    ...
                    <div className="p-6 pt-10">
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-700/50 flex items-center justify-between">
                        <span className="text-xs text-slate-500">
                          {build.runes.length} Recommended Runes
                        </span>
          ...
          {buildDetails.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-slate-500">
                We're working on curating the best builds for each class!
              </p>
            </div>
          )}
        </div>
